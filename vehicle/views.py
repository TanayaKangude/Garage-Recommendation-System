...
from .models import CustomerRequest, Mechanic
...
def request_page(request):
    if request.method == 'POST':
        ...
        CustomerRequest.objects.create(request_type=request_type)
        if request_type == 'garage_recommendation':
            return redirect('customer_recommend_request')
        elif request_type == 'service_enquiry':
            return redirect('customer_add_request')
    return render(request, 'vehicle/customer_request_choice.html')

def service_page(request):
    return render(request, 'vehicle/service.html')

def team_page(request):
    return render(request, 'vehicle/team.html')

def faq_page(request):
    return render(request, 'vehicle/faq.html')

def support_page(request):
    return render(request, 'vehicle/support.html')

def terms_page(request):
    return render(request, 'vehicle/terms.html')

def privacy_page(request):
    return render(request, 'vehicle/privacy.html')

def disclaimer_page(request):
    return render(request, 'vehicle/disclaimer.html')

def blog_page(request):
    return render(request, 'vehicle/blog.html')

def blogpost_page(request):
    return render(request, 'vehicle/blog_post.html')

def blogpost1_page(request):
    return render(request, 'vehicle/blog_post1.html')

def blogpost2_page(request):
    return render(request, 'vehicle/blog_post2.html')
...
def admin_add_vehicle_view(request):
    if request.method == 'POST':
        ...
        form = VehicleForm()
    return render(request, 'vehicle/admin_add_vehicle.html', {'vehicleform': form})
...
@login_required(login_url='customerlogin')
@user_passes_test(is_customer)
def customer_recommend_request_view(request):
    customer=models.Customer.objects.get(user_id=request.user.id)
    enquiry=forms.RequestForm()
    if request.method=='POST':
        enquiry=forms.RequestForm(request.POST)
        if enquiry.is_valid():
            ...
            enquiry_x.customer=customer
            enquiry_x.save()
            garages=Mechanic.objects.filter(vehicletype=enquiry_x.vehicletype)
            return render(request, 'vehicle/garage_list.html',{'garages':garages})        
    return render(request,'vehicle/customer_recommend_request.html',{'enquiry':enquiry,'customer':customer})
...
def contactus_view(request):
    ...
    if request.method == 'POST':
        ...
        if sub.is_valid():
            ...
            send_mail(str(name)+' || '+str(email),message,settings.EMAIL_HOST_USER, settings.EMAIL_RECEIVING_USER, fail_silently = False) # type: ignore
