...
class MechanicForm(forms.ModelForm):
    vehicletype = forms.ModelChoiceField(queryset=models.Vehicle_type.objects.all(), empty_label=None, widget=forms.Select)
    class Meta:
        model=models.Mechanic
        fields=['address','mobile','profile_pic','skill','vehicletype']
...
class RequestForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields=['vehicletype','vehicle_no','vehicle_name','vehicle_model','vehicle_brand','problem_description']
        widgets = {
        'problem_description':forms.Textarea(attrs={'rows': 3, 'cols': 30})
        # 'Requirement':forms.Textarea(attrs={'rows': 3, 'cols': 30})
        }
